...
function RaceCard({ raceName }) {
  ...
  if (!rows || rows.length === 0) {
    return (
      <div className="mb-8 border border-gray-800 rounded-xl overflow-hidden">
  ...
  return (
    <div className="mb-8 border border-gray-800 rounded-xl overflow-hidden">
      ...
      {/* ── Card header ── */}
      <div className="bg-gray-900 px-4 py-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-900/60 border-b border-gray-800">
              ...
              <th className="text-left px-4 py-2 text-gray-400 font-medium">1st SP (pts)</th>
              <th className="text-left px-4 py-2 text-gray-400 font-medium">2nd SP (pts)</th>
              <th className="text-left px-4 py-2 text-gray-400 font-medium">3rd SP (pts)</th>
              <th className="text-right px-4 py-2 text-gray-400 font-medium">SP Total</th>
              ...
              return (
                <tr
                  ...
                  className={`border-b border-gray-800 ${
                    rowHighlight || (idx % 2 === 0 ? 'bg-gray-950' : 'bg-gray-900/50')
                  }`}
                >
                  {/* Year */}
                  <td className="px-4 py-3 font-medium text-gray-200">
                    ...
                    {isHigh && rows.length > 1 && (
                      <span className="ml-2 text-xs text-emerald-400 font-normal">↑ high</span>
                    )}
                    {isLow && rows.length > 1 && (
                      <span className="ml-2 text-xs text-rose-400 font-normal">↓ low</span>
                    )}
                  </td>
      ...
      {/* ── Summary footer ── */}
      <div className="bg-gray-900 px-4 py-3 border-t border-gray-800 flex flex-wrap gap-6 text-sm">
        <div>
          <span className="text-gray-500 text-xs uppercase tracking-wide">Years in data</span>
          <div className="text-gray-200 font-medium">{rows.length}</div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="text-gray-200 font-medium">
            {highRow.year}{' '}
            <span className="text-emerald-400 font-bold">{highRow.spTotal.toFixed(1)}</span>
          </div>
        </div>
        <div>
          ...
          <div className="text-gray-200 font-medium">
            {lowRow.year}{' '}
            <span className="text-rose-400 font-bold">{lowRow.spTotal.toFixed(1)}</span>
          </div>
        </div>
      </div>
    </div>
