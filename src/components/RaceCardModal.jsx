...
function ParsedRaceCard({ race, index, onLoad, totalRaces }) {
  ...
  return (
    <div className="border border-gray-300 rounded-xl overflow-hidden mb-4">
      ...
      {/* Card header */}
      <div className="bg-gray-100 px-4 py-2.5 flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-gray-900 font-semibold text-sm">{race.raceName}</span>
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-500
                     text-gray-900 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
        >
          Load into Optimiser →
        </button>
      </div>
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-200 ${
                      pos ? 'bg-emerald-100' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                    }`}
                  >
                    <td className="px-3 py-1.5 font-mono text-gray-500">{r.gatePosition}</td>
                    <td className="px-3 py-1.5 text-gray-700">{r.horseName}</td>
      ...
      {/* Result summary: full */}
      {hasResult && (
        <div className="border-t border-emerald-900/40 bg-emerald-50/20 px-4 py-2 flex gap-4 flex-wrap">
          <span className="text-xs text-emerald-500 font-semibold">Full top 3 detected:</span>
          {race.result.map((r, i) => (
            <span key={i} className="text-xs text-emerald-700 font-mono">
    ...
    <div className="flex flex-col gap-4 h-full">
      <div>
        ...
        <ul className="text-gray-500 text-xs space-y-0.5 list-disc list-inside mb-3">
          ...
          <li>Fractional odds: <span className="font-mono text-gray-500">6/1</span>, <span className="font-mono text-gray-500">7/2 Fav</span>, <span className="font-mono text-gray-500">4/1 (2nd)</span></li>
          <li>Finish positions optional: <span className="font-mono text-gray-500">(1st)</span> <span className="font-mono text-gray-500">(2nd)</span> <span className="font-mono text-gray-500">(3rd)</span></li>
        ...
        <textarea
          ...
          className="w-full h-64 bg-gray-50 border border-gray-300 rounded-lg px-3 py-2
                     text-sm text-gray-700 font-mono resize-y
    ...
    return (
      <div className="flex flex-col gap-4">
        <div className="border border-rose-800 rounded-xl bg-rose-950/40 px-4 py-4 text-center">
          <p className="text-rose-600 font-semibold mb-1">No runners could be parsed.</p>
...
export default function RaceCardModal({ onClose, onLoadRace }) {
  ...
  return (
    ...
    >
      <div className="bg-white border border-gray-300 rounded-2xl w-full max-w-2xl shadow-2xl
                      flex flex-col max-h-[90vh]">

        {/* Modal header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div>
            <h2 className="text-gray-900 font-bold text-base">Paste Race Card</h2>
